...
output_path = ENV['BASE_PATH'] + "\\edueveryday\\ready\\#{folder_name}"
output_file = "#{ENV['BASE_PATH']}\\edueveryday\\voiceover.flac"
FileUtils.mkdir_p(output_path)
edu_videos_dir = ENV['BASE_PATH'] + '\\edueveryday\\video'
...
prompt = "Tell me about interesting historic event which happened on #{Time.now.strftime('%m-%d')} in under 60 seconds. Start with the words 'Today' and add date and year."
...
random_creepy_video = client.random_file_from_directory(edu_videos_dir)
random_stupid_footage = client.random_file_from_directory(stupid_footage_cutter_dir)

FileUtils.cp("#{edu_videos_dir}\\#{random_creepy_video}", "#{output_path}\\#{random_creepy_video}")
FileUtils.cp("#{stupid_footage_cutter_dir}\\#{random_stupid_footage}", "#{output_path}\\#{random_stupid_footage}")
FileUtils.cp(ENV['BASE_PATH'] + '//edueveryday//edueveryday.flp', "#{output_path}\\edueveryday.flp")
FileUtils.cp(ENV['BASE_PATH'] + '//edueveryday//outro.mp3', "#{output_path}\\outro.mp3")
